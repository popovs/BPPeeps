...
Dataset: one count record per station per survey date, `nrow(dat)` records. **`dat_p0`%** of the records are zeroes.
...
```{r, fig.cap="Best-fit model distribution of zeroes vs. observed proportion of zeroes (red line). The model over-estimates the amount of zeroes observed in our dataset."}
set.seed(101)
system.time(
...
Dataset: one count record per N/S division per survey date, `nrow(dat3)` records.
...
## Poisson and negative binomial models
fit_poisson <- glmmTMB::glmmTMB(predicted_wesa ~ n_s + year_c +  dos + I(dos^2) + (dos + I(dos^2) | year) + (1|year),
                                data = dat3, 
                                ziformula = ~0, # no zero inflation 
                                family = "poisson")
fit_binom1 <- update(fit_poisson, family = glmmTMB::nbinom1) 
...
## zero-inflation models
fit_zipoisson <- glmmTMB::glmmTMB(predicted_wesa ~ n_s + year_c +  dos + I(dos^2) + (dos + I(dos^2) | year) + (1|year),
                         data = dat3,  
                         ziformula = ~1,
                         family = "poisson")
fit_zinbinom1 <- update(fit_zipoisson, family = glmmTMB::nbinom1) 
...
bestfit_dat3 <- fit_zinbinom2
```

- Similar to above, best-fit model is a zi negative binomial model.
...
```{r, fig.cap="Best-fit model distribution of zeroes vs. observed proportion of zeroes (red line). The base model actually does a great job of estimating the correct proportion of zeroes!"}
plot.new()
set.seed(101)
system.time(
...
The residuals are also looking really good for the base model:
...
Since the base model seems to actually be doing a good job of predicting our zeroes correctly, lets add the full suite of important variables identified in Canham et al. (2021). 
...
build <- buildmer::buildglmmTMB(predicted_wesa ~ n_s + year_c + dos + I(dos^2) + 
                                  ...
                                  scale(v) + scale(windspd) + n_s:scale(flow),
                                dat3, 
