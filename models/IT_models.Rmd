---
title: "`r params$spp` `r params$model_set` models"
author: "Sarah Popov"
date: "`r Sys.Date()`"
output: pdf_document
params:
  spp: "DUNL"
  model_set: "ZINB" # "LME", "ZINB", or "GAM"
---
...
(dat_ns %>%
   ...
   dplyr::select(y, year, dos, n_s, elev_range, flow, total_precip, mean_temp, u) %>%
...
tgam_flow <- gam(y ~ n_s + s(flow) + s(year_c) + s(dos) + s(year, bs = "re"), 
             data = dat_ns, 
             family = tw, 
             method = "ML")

tgam_temp <- gam(y ~ n_s + s(mean_temp) + s(year_c) + s(dos) + s(year, bs = "re"), 
             data = dat_ns, 
             family = tw, 
             method = "ML")

tgam_tide <- gam(y ~ n_s + tide + s(year_c) + s(dos) + s(year, bs = "re"), 
             data = dat_ns, 
             family = tw, 
             method = "ML")

tgam_ta <- gam(y ~ n_s + s(elev_range) + s(year_c) + s(dos) + s(year, bs = "re"), 
             data = dat_ns, 
             family = tw, 
             method = "ML")

tgam_tp <- gam(y ~ n_s + s(total_precip) + s(year_c) + s(dos) + s(year, bs = "re"), 
             data = dat_ns, 
             family = tw, 
             method = "ML")

tgam_u <- gam(y ~ n_s + s(u) + s(year_c) + s(dos) + s(year, bs = "re"), 
             data = dat_ns, 
             family = tw, 
             method = "ML")
...
modList <- list('NS + YEAR + DOS' = tgam0,
                 'FLOW + NS + YEAR + DOS' = tgam_flow,
                 'AMP + NS + YEAR + DOS' = tgam_ta,
                 'TEMP + NS + YEAR + DOS' = tgam_temp,
                 'PRECIP + NS + YEAR + DOS' = tgam_tp,
                 'U + NS + YEAR + DOS' = tgam_u,
                 'Full ME tgam' = tgamf0,
                 'Full ME tgam + NS:FLOW' = tgamf0_flow,
                 'Full ME tgam + NS:AMP' = tgamf0_ta, 
                 'Full ME tgam + NS:TEMP' = tgamf0_temp,
                 'Full ME tgam + NS:PRECIP' = tgamf0_tp,
                 'Full ME tgam + NS:U' = tgamf0_u,
                 'Full ME tgam + All interaction terms' = tgamf1)
```
...
top_model <- modList[[rownames(as.data.frame(tab))[1]]]
```
...
tab2 <- tab2[!(tab2$mod %in% c("Full tGAM + All interaction terms", "Full ME ZINB + All interaction terms")),]
...
can_avg <- !(tab2[1,4] > 0.95)
```

## The top 5 `r params$model_set` models

The table below pulls out the top 5 `r params$model_set` models, *excluding* the model with all interaction terms. In other words, it ranks the models by the NS interaction terms that bring the candidate model the closest to the 'true' model. `r if(!can_avg) paste("As was the case with the full model, the top model carries nearly all the AIC weight (>95%). This set of candidate models is not suitable for model averaging.")`

```{r top5 table}
knitr::kable(tab2)
```

`r if(can_avg) paste("The top 5", params$model_set, "models were then averaged together and presented below.")` `r if(params$model_set == "GAM") paste("Note that model averaging with GAMs may not make all that much sense, because it's averaging the smoothing functions of each parameter. Interpret with caution.")`

```{r top 5 model avg plot, eval=can_avg, fig.cap="Plot of averaged model coefficients from the top-5 models (excluding the full model with all coefficients and interaction terms)."}
sjPlot::plot_model(MuMIn::model.avg(top5mods(tab2), subset = cumsum(weight) <= 0.95))
```

```{r top 5 model avg summary, eval=can_avg, fig.cap="Model summary of the averaged top-5 models (excluding the full model with all coefficients and interaction terms).", include=FALSE}
print(summary(MuMIn::model.avg(top5mods(tab2), subset = cumsum(weight) <= 0.95)), digits = 1)
```
...
\newpage
# Top `r params$model_set` model

## Model summary

```{r summary top model}
print(summary(top_model), digits = 1)
```

\newpage
```{r plot zinb coefficients, eval=(params$model_set == "ZINB"), fig.cap="Standardized coefficient effect sizes from the full ZINB model."}
...
```{r plot GAM effects, eval=(params$model_set == "GAM"), fig.cap="Estimated smooths from the top GAM model.", fig.width=8, fig.height=11}
gratia::draw(top_model)
```


\newpage

```{r top model diagnostics, child = "child_docs/diagnostics.Rmd"}
```